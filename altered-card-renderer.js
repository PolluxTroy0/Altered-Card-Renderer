...
(function (global) {
  ...
  const RESOURCES = {
    ...
    //   {imgfolder}   → image quality folder: "illustrations" (default) or "illustrations_hd" (?q=hd in page URL)
    //   {imgext}      → image extension: "webp" (default) or "jpg" (?q=hd in page URL)
  ...
  function _apiToCardJson(apiJson, mapping) {
    ...
    if (_opts.useApiBackground === false && _opts.backgroundUrl) {
      ...
      // COREKS intermediate fallback — try CORE illustrations folder before the generic backup
      if (apiJson.set?.reference === "COREKS" && _opts.backgroundUrl) {
        const coreRef   = (apiJson.reference ?? "").replace("_COREKS_", "_CORE_");
        const coreParts = coreRef.split("_");
        const coreBgref = coreParts.length >= 6
          ? coreParts.slice(0, 5).join("_") + "_" + coreParts[5].charAt(0)
          : coreRef;
        const rawMid = _opts.backgroundUrl
          .replace("{imgfolder}",   imgFolder)
          .replace("{imgext}",      imgExt)
          .replace("{ref}",         coreRef)
          .replace("{locale}",      lang)
          ...
          .replace("{id}",          coreRef)
          .replace("{set}",         "CORE")
          .replace("{bgref}",       coreBgref)
          .replace("{framesuffix}", framesuffix);
        bgMidUrl = (proxy && rawMid) ? proxy + "?img=" + encodeURIComponent(rawMid) : rawMid;
      }
    } else {
    ...
    if (bgUrl || bgMidUrl || bgBkpUrl) cardJson._urls = { bg: bgUrl || null, bgMid: bgMidUrl || null, bgBkp: bgBkpUrl || null };
  ...
  function _buildStateFromJson(config, cardJson) {
    ...
    for (const el of elements) {
      ...
      settings[el.id] = {
        ...
        fontSizeByLength: d.fontSizeByLength ?? fr.fontSizeByLength ?? fto.fontSizeByLength ?? ft.fontSizeByLength ?? g.fontSizeByLength ?? null,
        ...
        bgShadow:    d.bgShadow   ?? fr.bgShadow   ?? fto.bgShadow   ?? ft.bgShadow   ?? g.bgShadow   ?? null,
      ...
      if (el.isBiome) {
        ...
        if (biomeCfg.bgShadow   != null && d.bgShadow   == null) settings[el.id].bgShadow   = biomeCfg.bgShadow;
      }
    }
  ...
  async function _loadCardAssets(state, cardJson) {
    ...
    jobs.push(
      loadImg(imgs.bg || urls.bg || null)
        .then(img => img || loadImg(urls.bgMid || null))
  ...
  // zone2 = { fromLine, x, maxWidth } — optional second zone (different width/x after N lines)
  // measureOnly = true — skip all drawing, just return lineCount (used for font auto-fit)
  function _drawRichText(ctx, runs, x, y, maxWidth, lineHeight, fontSize, color, fontNames, zone2 = null, measureOnly = false) {
  ...
  function _renderCard(state, _canvas, ctx) {
    ...
    const _scale = state.canvasScaleExport ?? 1;
    const W = CARD_W * _scale, H = CARD_H * _scale;
    ...
    for (const el of state.elements.filter(e => e.isBiome)) {
      ...
      // Drop shadow under the badge (from s.bgShadow only — no textShadow fallback).
      // save/restore isolates the shadow so the frame overlay drawn next isn't affected.
      ctx.save();
      const sh = s.bgShadow;
      if (sh) {
        ctx.shadowColor   = sh.color   ?? "transparent";
        ctx.shadowBlur    = (sh.blur    ?? 0) * _scale;
        ctx.shadowOffsetX = (sh.offsetX ?? 0) * _scale;
        ctx.shadowOffsetY = (sh.offsetY ?? 0) * _scale;
      }
    ...
    for (const el of textEls) {
      ...
      const x        = (s.x / 100) * W;
      const y        = (s.y / 100) * H;
      const fontSize = Math.round(s.fontSize * _scale);
      ...
      if (s.textShadow) {
        ...
        ctx.shadowBlur    = (s.textShadow.blur     ?? 0) * _scale;
        ctx.shadowOffsetX = (s.textShadow.offsetX  ?? 0) * _scale;
        ctx.shadowOffsetY = (s.textShadow.offsetY  ?? 0) * _scale;
      }
      ...
      if (el.inputType === "richtext") {
        ...
        if (s.fontSizeByLength?.length) {
          const plain = val.replace(/<[^>]*>/g, '').replace(/&[^;]+;/g, 'X').trim();
          const len = plain.length;
          for (const step of s.fontSizeByLength) {
            if (len >= step.from) effectiveFontSize = step.size * _scale;
          }
        }

        const lineH = effectiveFontSize * (s.lineHeight || 1.4);
        const zone2 = (s.maxLines > 0 && s.x2 != null && s.maxWidth2 != null)
          ? { fromLine: s.maxLines, x: (s.x2 / 100) * W, maxWidth: (s.maxWidth2 / 100) * W }
          : null;
        _drawRichText(ctx, runs, x, y, maxPx, lineH, effectiveFontSize, s.color || "#ffffff", state.fontNames, zone2);
      } else {
    ...
    for (const el of state.elements.filter(e => e.inputType === "herostat")) {
      ...
      const rW     = (s.rectW    || 18) * _scale;
      const rH     = (s.rectH    || 14) * _scale;
      const gap    = (s.rectGap  || 5)  * _scale;
      const radius = (s.rectRadius || 3) * _scale;
      if (count > 0) {
    ...
    if (!adminTextEl?.infoLinePart) {
      ...
      if (atV && atS?.visible !== false) {
        ...
        const aFs = Math.round((atS?.fontSize ?? 11) * _scale);
        ctx.save();
    ...
    (function () {
      ...
      async function _flushBatch() {
        ...
        for (const e of entries) {
          ...
          if (!data?.reference) {
            _cardErrCanvas(e.element, e.ref, "error");
            continue;
          }
      ...
      class AlteredCardElement extends HTMLElement {
        ...
        static async _loadSingle(element, ref, locale, collection) {
          ...
          if (!data?.reference) {
            _cardErrCanvas(element, ref, "error");
            return;
          }
          data.forge = { collection, lang: locale };
          let canvas;
          try {
            ({ canvas } = await AlteredRender.mountFromApi(element, data, undefined, { _resolvedProxy: _proxyBase }));
          } catch (err) {
            _cardErrCanvas(element, ref, "error");
            console.error("[altered-card] render failed", ref, err);
            return;
          }
